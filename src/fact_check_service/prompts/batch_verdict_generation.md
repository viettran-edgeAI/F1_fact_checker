...
Task: Decide a verdict for each supplied claim using only that claim's supplied evidence.

Return strict compact JSON only. Do not include markdown, comments, prose, reasoning, citations outside JSON, or trailing commas. Keep each summary short.
...
Grounding rules:
- Produce exactly one verdict object for every input item.
- Preserve each input claim_id exactly.
- Use only the evidence attached to that same input item.
- Do not use memory, outside knowledge, assumptions, or unstated facts.
- If evidence is insufficient, ambiguous, stale for a current claim, or not directly relevant, return verdict "unverified".
- If a claim was classified as unsupported, return verdict "unsupported" unless the supplied evidence makes it checkable.
- Every factual statement in summaries and corrections must be grounded in supplied evidence.
- Do not invent dates, race names, statistics, quotes, penalties, standings, or source details.
- Omit evidence-by-evidence analysis.
