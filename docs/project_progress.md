...
- Structured-route claims now pass through a Gemma rewrite / normalization step before local retrieval so season, year, race, and team context stays attached to standalone claims.
...
- structured claims that need local evidence pass through Gemma rewrite / normalization before structured retrieval
